...
gulp.task('build:templates:watch', () => {
    return watch(config.sources.templates, batch((e, done) => run('build:templates', 'build:scripts', done)));
});
...
gulp.task('build:watch', done => {
    run(['build:scripts:watch',
        'build:templates:watch',
        'build:styles:watch',
        'build:assets:watch',
        'build:fonts:watch',
        'build:index:watch',
        'build:sass:watch'], done);
});
...
gulp.task('build:sass:watch', () => {
    return watch(config.sources.scssall, batch((e, done) => run('build:sass', done)));
});
